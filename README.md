...
- ✅ **Phase 9 第七轮：抽取关掉思考 + 降级可见**（抽取这一步**给推理模型关掉思考**
  （`THINKING_OFF`，经新增的可选 `extra_body` 透传，默认不发、parity 铁律不受影响）：
  推理的 `reasoning_content` 与正文共用 `max_tokens`，而该步不需要想那么久 —— 实测同一次抽取
  推理吃掉 19474 字、正文只剩 1418 字且断在半句，12 个知识点的好答案变成 **0 个**。
  另加 `salvage_json`：输出被截断时救回**已完整**的元素（`parse_json_object` 一字未动）；
  预算 8192 → 16384（关思考后实测只需 1293，加宽是免费保险）；提纲那一路补上它**压根没传**的
  `max_tokens=4096` + 关思考。前端在图上有节点、且每个节点都没有关键词时给出提示并指路
  设置页「进阶」——降级产物此前看起来和正常图**完全一样**，生成日志一走就再无痕迹）
