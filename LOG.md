...
## 走查反馈第七轮 —— 抽取被截断：关掉思考 + 抢救 JSON + 让降级可见（2026-09-13）

用户的反馈是一句直觉，但指向的是这一轮真正的 bug：

> 现在这个真的是对的吗 我为什么感觉他现在的只是图谱的生成不太合理
> 比如说我找rerank技术 知识图谱中应该节点可能出现rerank有详细的技术方法更合理

### 先量，不猜：那张 rerank 图不是「抽得差」，是**根本没抽**

三处证据（每一处都是查出来的，不是推出来的）：

1. 库里那张 rerank 图（`0dc5a5`，20:47 那次）**4 个节点全是 `type="章节"`、`key_points` 全空**、
   `summary` 就是研究计划每步的 `purpose`（「重排技术的主流方案有哪些」）→ 它是 `nodes_from_plan`
   的**降级产物**。`research_runs` 那次 `report` 长度 0，而 `GET` 出来的图 `status=ready`。
2. 用真实存下的 plan + evidence（35 条）**原样重跑**抽取节点：
   `finish_reason=length`，**推理 19474 字 / 正文 1418 字**，JSON 写到第 7 个节点断在半句
   （`…"可用LLM judge对难负例标注"]` 少了收尾的 `}`）→ `parse_json_object` → `None` → **0 个节点**。
3. 同一份资料、同一个 prompt，**关掉思考**后：`finish_reason=stop`、推理 0、正文 3386 字、
   **12 个节点** —— `交叉编码器重排 / 难负样本挖掘 / 知识蒸馏训练 / 合成数据生成 / 领域微调重排 /
   LLM列表重排 / 级联重排 / 智能体重排 / 多跳推理重排 / 重排失效模式 / 重排鲁棒性诊断 / 在线A/B验证`，
   各 3 个关键词 + 真实前置关系。**这就是用户想要的那张图。**

所以第六轮「删掉报告」的方向没错，错在**这个结构化抽取调用没关推理**：8192 的预算里推理占九成，
而抽取（把已经读过的证据整理成结构化清单）根本不需要想那么久。两个次生缺陷把它放大成静默故障：

- **截断的 JSON 整份丢弃**，而不是救回已经完整的那部分（12 个好节点 → 0 个）；
- **降级产物长得像正常图谱** —— 节点数、边数、`status=ready` 都对，生成日志一走就再没有任何痕迹。

### 术语澄清：`max_tokens` 卡的是「想了多久」，不是「读了多少」

用户追问过预算到底是什么，这里写清楚，因为下一轮很容易照着错误的直觉去调它：

> `max_tokens` **只卡输出**。输入（那次抽取的 prompt 是 9275 字）一分都不占它。
> 但推理模型的输出被劈成 `reasoning_content` + 正文两半，**两半共用同一个 `max_tokens`**。
> 危险程度于是与「读了多少资料」无关，与「模型想了多久」成正比 —— 同一份 9275 字的输入，
> 有时推理吃 19474 字（正文只剩 1418，断在半句），有时吃 20369 字（**正文一个字都没有**）。
> 后一种连抢救的余地都没有：没有东西可救。这就是为什么**关掉思考是主修，救 JSON 只是保险**。

### 三处改动

**① 关掉思考（主修）。** 新增可选参数 `extra_body`（`LLMClient.complete` / `ChatClient.complete` /
`ProviderClient.complete` / `ModelGateway.complete`），沿用 `max_tokens` 那条既有约定 ——
**仅非 None 时才写进 body** —— 所以 B 轨那条逐字节 parity 铁律（`httpx.MockTransport` 比较外发
JSON）一个字节都没动。方言常量 `THINKING_OFF = {"thinking": {"type": "disabled"}}` 定在
`llm/providers.py`（说 SDK 那门语言的那一层），agent 层只把它当**不透明字典**往下递。
端点实测接受这个方言；`enable_thinking=False` 与 `chat_template_kwargs` 两个候选**都仍在推理**
（分别 1051 / 669 字），所以只认这一个。

**② 抢救被截断的 JSON。** `llm/json_utils.py` 新增 `salvage_json(text)` = `parse_json_object`
+ 「在最后一个完整的取值边界处切一刀、补上收尾括号、再 parse」。
`parse_json_object` 一字未动。新函数在写作过程中被自己的对抗夹具咬出三个洞，都补了：

- `parse_json_object` 在残缺输入上**不是 None 而是捡回一部分**（外层对象截断在「数组闭合之后、
  `}` 之前」→ 交回那个内层数组；裸数组截断 → 交回**第一个元素**），所以「parsed 不是 None 就早退」
  会把最该修的输入整条跳过；
- 完整裸数组 `[{…}, {…}]` 被判据切成第 1 位的 `{`，于是**元素一冒充整份结果**；
- `{"outline": ["甲", "乙", "丙` 这种形状**一个括号都没闭合** —— 只认括号就整份救不回来，
  而它恰恰是「数组装字符串」的**必然**截断形态。于是「数组里的字符串闭合」也算作边界。

预算同步 8192 → **16384**（`EXTRACT_MAX_TOKENS`）。这不是「调大来治症状」：额度只卡输出、不预留，
关掉思考后同一次抽取只需 **1293** 个输出 token —— 加宽是免费的保险，不是堵洞。

**③ 让降级看得见。** `web/js/views/graph.js` 新增纯函数 `degradedHTML(g)`，命中时在
`header` 与 `.canvas` 之间插一条 `.notice`，指路**设置页 →「进阶」**。

判据取的是**症状**（有节点 && 每个节点的 `key_points` 都空），不是 `type === '章节'`：
`type` 在正常抽取路径里也是模型自己写的自由文本，它完全可能给某个知识点写「章节」，
那时一张好好抽出来的图会被打上「没抽过」的标签。空关键词既是两条降级路径
（`nodes_from_headings` / `nodes_from_plan`）的共同产物，又是正常抽取几乎不会出现的形状。
**没有**为了这条提示把顶栏那个重生成按钮加回来 —— 设计稿 §23 撤它是因为重跑会按新的
`order_index` 重算 `note_path`，用户写在正文里的「## 我的笔记」会成孤儿。

### 提纲那一路顺手补了两处（同类，但**没有**单独实测）

`learning/outline.py::generate_outline` 这次调用**压根没传 `max_tokens`**（走 provider 默认），
而实测它推理 2571 字 / 正文 88 字（**29 倍**）—— 是同类里最脆的一处。补上 `OUTLINE_MAX_TOKENS = 4096`
与 `THINKING_OFF`，解析改用 `salvage_json`。

**代价与残留风险（不假装没有）**：

- 关掉思考对抽取是**纯赚**（本来就不需要推理），但提纲的关思考是**照同类推理的**，没单独验过
  质量有没有变浅。提纲是短清单，风险低，但这条是推断不是实测。
- 若某个 provider **不认** `thinking` 这个键而回 400，抽取会走它既有的 `except` → 交白卷 →
  降级成计划。也就是说「方言被拒」的失败模式退回到第六轮的样子（可见，但抽不到）——
  新增的那条提示正好覆盖这种情况，这是它存在的第二个理由。
- **不改讲解**（`session.py`）：实测推理 4575 / 正文 1233、2816/4096 刚好够，上一轮的空回答闸门
  让它只会报错重试、不会静默坏；而且推理对「讲课」的质量可能真有用，关掉可能讲得更浅。
- **不改 loop 模式的 `synthesize`**：报告是给用户读的长文，推理有价值。
- **不加 DB 列持久化降级原因**：判据从既有列（`key_points`）就能推出来，零迁移，而且**对已经坏掉的
  那张图立刻生效**。
- `salvage_json` 对「推理把正文吃到 0 字」这一种**无能为力** —— 没有东西可救。只有关思考能治它。
- 那张现存的坏图**必须重新生成一次**才有救（设置页 →「进阶」），代价是同名节点的笔记文件按新顺序
  重命名 —— 那 4 个文件是自动生成的，风险低。

### 测试（有牙，逐条验过）

`pytest -q` —— **685 passed**（第六轮基线 648）；`node --test tests/js/` —— **370 pass / 0 fail**（基线 366）。

新增/改写：`tests/test_json_salvage.py`（**新，27 条**：8 条「好输入一个字节都不动」的参数化矩阵
（含裸数组、围栏、前缀废话、垃圾、空串）、外层对象截断、字符串中途截断、嵌套数组中途截断、
提纲形状、**「救的是最长前缀不是第一个能解析的」**、围栏+截断、前缀废话+截断、裸数组截断、
完整裸数组不被切、数组元素不冒充整份结果、6 条「救不回来仍是 None」、只补括号不造键）；
`tests/test_llm_gateway_providers.py` + 3 条（**默认外发 body 里没有 `extra_body`/`thinking` 键**
—— parity 铁律的另一半；传了就**平铺**成 `{"thinking": {"type": "disabled"}}`；流式路径不带）；
`tests/test_llm_gateway.py` + 2 条（透传 / 没给时 provider 收到的仍是 `None`；
**fallback 到第二个 provider 时方言跟着走**）；`tests/test_agent_extract.py` + 2 条
（`seen_extra_bodies == [None, None, THINKING_OFF]` —— 「只有抽取那一次」也一并钉住；
截断回复仍救回完整的那 1 个节点）；`tests/test_learning_outline.py` + 3 条
（4096 与 `THINKING_OFF` **钉字面值**、提纲截断救回前两条、第一条就断时仍是 `[]`）；
JS `views.test.mjs` + 4 条（全空关键词 → 出提示且**指向设置页**、且**没有**把 `id="regen"` 加回来；
有一个带关键词 → 不出提示但图一个盒子不少；空图 → 不出提示；**判据认症状不认类型** ——
`chain`（按标题分章那条降级路径）也提示、而类型写着「章节」但关键词有内容时**不**提示）。

**变异验证**（改坏 → 确认变红 → 改回，每条都只剩对应的那几条红）：

| 改坏什么 | 结果 |
|---|---|
| `degradedHTML` 的 `every` → `some` | JS **1 红**（「有一个带关键词就不出提示」那条） |
| 拿掉 `extract_node` 的 `extra_body=` | **1 红**（`seen_extra_bodies[2]` 是 None） |
| `_salvage_from` 的 `reversed(checkpoints)` → 正序 | **5 红**（含「完整答案不被切成元素一」） |
| 补收尾括号的 `reversed(open_stack)` 拿掉 | **3 红**（含提纲那条真·调用点用例） |
| `_checkpoints` 不再把「数组里的字符串闭合」当边界 | **3 红**（提纲形状 + 提纲真·调用点） |

### 人工验收缺口（需要浏览器 + 真实模型）

1. **重跑一次 rerank 主题**（设置页 →「当前探索」→「重新研究并生成」）：节点应从 4 变 ~12、
   名字是 `交叉编码器重排` 这类**具体技术方法**、每个都有关键词（不是空的）；
2. 打开其中任一节点 → **提纲生成得出来**（这一路补了预算与关思考）；
3. 那张**旧**图在重跑之前应当显示「每个知识点都没有关键词…」的提示；
4. 生成日志里不再出现「没有抽取出知识点」；
5. 若模型/provider 换了、方言失效 → 抽取交白卷 → 图上出现同一条提示（可见的失败，不是静默的）。

### 补记：那条提示**指错了地方**（2026-09-13 晚，用户走查）

用户第一条反馈就是它：

> 1、前端没有显示进阶的选项

提示让人去「设置页 →「进阶」」，而**设置页里根本没有「进阶」**——整个 `views/settings.js`
连这两个字都 grep 不到（「进阶」只出现在 `views/graph.js` 自己的注释与这条提示里，
是我写提示时凭空造出来的**分区名**）。真正的位置是设置页「当前探索」卡里的 `#st-regen`
（「重新研究并生成」），代价写在按钮下面那段 `.s-note` 里。

**为什么会写错**：文案在 `graph.js`、入口在 `settings.js`，两个文件；而计划里这一区原本
就叫「进阶」（`graph.js` 顶栏那段旧注释也这么写着），后来名字没落进设置页，提示却照着旧计划写了。
**指错路的提示比不指路更糟**：用户照着找、找不到，只会认为「这系统就是这样」——
而这条提示存在的全部意义就是告诉他「这张图不是抽出来的，你可以重来」。

修两处：提示改成 `设置页 →「当前探索」→「重新研究并生成」`；`graph.js` 顶栏注释里同样的
「进阶」一并改成「当前探索」卡（`#st-regen`）。

**加了一条机械守卫**（`views.test.mjs`）：把提示里用「」引起来的**每个名字当指针验** ——
它必须出现在设置页渲染出来的**有名字的东西**里（`<h3>` 分区标题 + `<button>` 文字）。
第一版写的 `page.includes(名字)` 是**恒真**的：「当前探索」被删除按钮的标签「重置当前探索」
兜住了，把卡片标题改成「我的主题」那条测试也不红（实测过，所以才有这一层）。

| 改坏什么 | 结果 |
|---|---|
| 提示改回「设置页 →「进阶」」 | JS **2 红**（指路线 + 守卫线，报「设置页上没有这个名字」） |
| 设置页卡片标题「当前探索」→「我的主题」 | **1 红**（守卫报出来） |
| 设置页按钮文字「重新研究并生成」→「重跑一次」 | **1 红** |

`node --test tests/js/` —— **371 pass / 0 fail**（本轮基线 370）；`pytest -q` **685 passed** 不变
（这次只动了提示文案、注释、JS 测试，Python 侧一个字节没碰）。

---

## 走查反馈第八轮 —— 右栏「学习状态」回到左对齐（2026-09-13）

> 关于右侧的学习状态栏，能否调整一下调整到和其他的位置一样左对齐 距离线有一定的距离
> 并且继续学习按钮的长度适当缩短

三条要求里,**第二条不是审美问题,是一个真的排版漏洞**(下面②)。

### ① 左对齐 —— 推翻第二轮的决定

第二轮用户的原话是「「学习状态」那一栏贴齐左边,**不符合居中**」,当时照做改成居中,理由写在
CSS 注释里(「三样都靠左时右半栏是一大片空白,读起来像没排过版」)。第八轮的实际观感相反:
**整个右栏只有这一块是居中的** —— 上面是标题条与「系统建议点亮」那条提示,下面是按钮行、
再往下是 `.scroll` 里的「概览」「相关节点」「来源」,全都左对齐;夹在中间的一块居中排版
反而像**没对齐**;而且居中之后,芯片那一行的左边缘与按钮的左边缘**互不重合**,谁都不挨着谁。
设计稿 §17 那张右栏草图本来也是左对齐的(进度条、「8 / 15」、「学习中」都钉在左边)。

两轮结论相反时按最新一轮做,但**两轮的理由都要留在注释里** —— 只留一半,下一个人会当成笔误改回去。

### ② 真正的病根:这一段**不在 `.scroll` 里**,于是少了一条左内边距

除它以外的块,横向内边距要么来自自己(`.p-head` / `.p-actions` / `.reason` 各自写了 17px),
要么来自容器(`#panel .scroll` 的 `padding: 13px 17px`,「概览」「相关节点」都在里面)。
而 `views/inspector.js:560` 把 `statusHTML(n, action)` 拼在 **`#panel` 的直接子元素**位置上
(夹在 `.p-actions` 与 `.scroll` 之间,常驻不滚)、自己又没写内边距。后果是这一块的内容
(尤其是那个通栏按钮)**贴着左边那条分隔线**,而它上下两块都缩进了 17px。用户说的
「距离线有一定的距离」就是这一条。**整个右栏只有这一块没有那条边距。**

修法是给 `.sec-status` 自己补 `padding-left/right: 17px`。**居中的写法把这个症状盖住了一半**:
芯片行居中,看不出贴边;通栏按钮一露,就看见了 —— 表面修法只遮一半,这也是这次要回头找结构原因
的原因。

### ③ 按钮 `width: 100%` → `60%`

右栏内容宽 266~306px,于是按钮约 160~184px。**没有缩到与「标记为已掌握」同级的小按钮**
(设计稿 §18 说它是这一栏的主要动作),但左对齐之后通栏的横条会比上面那行(芯片 + 一句说明)长出一大截。

### 测试(有牙,逐条验过)

`tests/test_api.py::test_inspector_status_section_reads_left_aligned_like_its_neighbours`
(重写自 `..._is_centered`,那条钉的是被推翻的旧结论)。**内边距那半条不写死 17px** ——
它取 `.scroll` 的横向内边距来比:两个数各写各的,就是下一次「和别的地方对不齐」。

| 改坏什么 | 结果 |
|---|---|
| `.sec-status` 改回 `text-align: center` | **1 红**（「又居中回去了」） |
| `.st-row` 的 `justify-content` 改回 `center` | **1 红**（「芯片那一行还在居中」） |
| 按钮改回 `width: 100%` | **1 红**（「又变回通栏了」） |
| 去掉 `padding-left/right` | **1 红**（「不自己补上就会贴着分隔线」） |
| 把 `.scroll` 的内边距改成 22px(两处脱节) | **1 红**（两边不一致,跨规则比对真的在工作) |

`pytest -q` —— **685 passed**;`node --test tests/js/` —— **371 pass / 0 fail**(两条基线都没动)。

### 人工验收缺口

这一轮的结论**全部来自读 CSS 和代码,没有在浏览器里看过** —— 六成宽到底顺不顺眼、
17px 是不是够,只能靠眼睛。测试钉住的是「这三个数有没有被改回去」,不是「好不好看」。
